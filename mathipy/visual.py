...
visual_models = [
    ...
    "picture_graph", "dot_plot", "histogram", "circle_graph", "scatter_plot",
    "box_plot", "stem_leaf_plot",
    "table", "expression_frame", "polygon", "circle_figure", "angle_figure",
    "solid_figure", "net_diagram", "area_model", "array",
    "tape_diagram", "pattern_visual", "tree_diagram",
    "venn_diagram", "measurement_tool",
    "picture", "text_only",
]

visual_functions = ["no_visual", "essential", "decorative"]

visual_function_definitions = {
    "no_visual": "the item carries no figure at all; use this exactly when the type is text_only",
    ...
    "decorative": "the item can be answered from the text alone; this covers both a figure that restates what the text already states and a figure that depicts none of the mathematics the item asks about",
}

visual_model_definitions = {
    "number_line": "a line marked with ordered numeric values or tick marks (number line diagram); includes a double number line, which is two such lines in parallel; one of the three visual fraction models, with area_model and tape_diagram",
    "coordinate_plane": "a grid with two perpendicular axes on which points, shapes, lines, or curves are plotted, including the graph of a function or equation; choose line_graph or scatter_plot when the plotted points are observed data values; the complex plane is one of these",
    "bar_graph": "separated rectangular bars whose lengths represent category values",
    "line_graph": "data points connected by segments over an ordered axis such as time; choose over coordinate_plane when the points are observed data values rather than a mathematical relationship",
    "picture_graph": "repeated icons or pictures grouped into labelled categories so that counts can be read off (picture graph); choose picture when the drawings are not grouped into categories",
    "dot_plot": "dots stacked above a number line showing frequency; the Standards use 'line plot' through grade 5 and 'dot plot' from grade 6",
    ...
    "circle_graph": "a circle divided into labelled sectors whose angles carry proportions — a pie chart, or a spinner whose sectors carry outcomes; choose circle_figure when the circle is studied as a shape rather than divided into sectors, and area_model when the parts are equal and unlabelled",
    "scatter_plot": "unconnected points plotted against labelled axes, whether or not the horizontal axis is numeric; choose line_graph when the points are joined by segments; a residual plot is one of these",
    ...
    "table": "rows and columns of values or labels without pictorial encoding; includes two-way tables, ratio tables, and a matrix",
    "polygon": "a closed plane figure bounded by straight sides, including a composite region made of joined straight-sided shapes; choose area_model when the rectangle is partitioned to show a product or parts of a whole, array when the figure is rows and columns of separate objects, coordinate_plane when the figure is drawn on axes, pattern_visual when it repeats or grows in a sequence, solid_figure when it is drawn with depth, circle_figure when a curve rather than straight sides bounds the region, angle_figure when the lines leave the region open, and measurement_tool when a graduated instrument is shown against the figure; a workspace whose marked points are the vertices of a figure to be constructed takes the type of that figure",
    "expression_frame": "an arithmetic expression or vertical computation drawn with empty boxes to be filled, with or without number or digit cards supplied for placement; choose table when the boxes form labelled rows and columns of values",
    "circle_figure": "a circle or circular region studied as a shape, with radius, diameter, arc, or central angle marked; choose area_model when the circle is partitioned into parts of a whole, circle_graph when the sectors carry data values, venn_diagram when circles overlap to show set membership, and polygon when straight sides rather than a curve bound the region",
    "angle_figure": "an angle, ray, line, or line segment construction where no closed region is the object of study; choose polygon when the lines close a region and measurement_tool when a protractor or ruler is shown against the figure; marked points alone are not a construction, and a workspace whose points are the vertices of a figure to be constructed takes the type of that figure",
    "solid_figure": "a three-dimensional object drawn with depth or perspective, including a cross-section of one",
    "net_diagram": "a flattened, unfolded pattern of a three-dimensional figure; choose over solid_figure when unfolded",
    "area_model": "a region — rectangle, circle, or other shape — partitioned into equal or labelled parts to show a product or parts of a whole; choose over array when the display is a partitioned region rather than countable objects, and over polygon or circle_figure whenever the partition is what the question is about; one of the three visual fraction models, with tape_diagram and number_line",
    "array": "discrete objects arranged in equal rows and columns; choose over area_model when the elements are countable objects",
    "tape_diagram": "segmented strips or side-by-side bars representing quantities and their relationship (strip or bar model); one of the three visual fraction models, with area_model and number_line",
    "pattern_visual": "a repeating or growing sequence of shapes or objects",
    "tree_diagram": "branching lines showing outcomes, factors, or hierarchies; choose over polygon and angle_figure when the lines branch to organise possibilities rather than forming a shape",
    "venn_diagram": "overlapping circles showing set relationships; choose over circle_figure when the circles overlap to show membership rather than being the object of study",
    "measurement_tool": "a drawn instrument bearing a graduated scale or dial from which a value is read — ruler, protractor, thermometer, clock face, speedometer, weighing scale, or pan balance; choose over number_line when the scale belongs to a drawn instrument rather than a bare line, and over polygon, circle_figure, and angle_figure when the instrument is shown against a figure to be measured; choose picture for a device that displays digits rather than a scale, such as a calculator; a scale drawing takes the type of the figure it draws, its scale key being a legend rather than an instrument; choose the shape's own type when the instrument's outline rather than its reading is the object of study",
    "picture": "a drawing or photograph of real-world objects in which any quantity is carried by a label, caption, or price tag attached to an object rather than by the geometry of the drawing itself; choose picture_graph when the objects are grouped into labelled categories so that counts can be read off, measurement_tool when an object bears a graduated scale to be read, area_model when an object is partitioned into parts of a whole, and solid_figure when the object is drawn as a geometric solid to be measured",
...
visual_model_groups = {
    "polygon": "figural", "circle_figure": "figural", "angle_figure": "figural",
    "solid_figure": "figural", "net_diagram": "figural",
    "number_line": "quantity_model", "tape_diagram": "quantity_model",
    "area_model": "quantity_model", "array": "quantity_model",
    "coordinate_plane": "coordinate",
    "bar_graph": "data_display", "line_graph": "data_display",
    "picture_graph": "data_display", "dot_plot": "data_display",
    ...
    "table": "symbolic", "expression_frame": "symbolic",
    "picture": "depictive", "measurement_tool": "depictive",
    "pattern_visual": "sequential",
    "tree_diagram": "relational", "venn_diagram": "relational",
    "text_only": "no_visual",
}
...
visual_model_signs = {
    "expression_frame": "symbol",
    "polygon": "icon", "circle_figure": "icon", "angle_figure": "icon",
    "solid_figure": "icon", "net_diagram": "icon",
    "area_model": "icon", "array": "icon",
    "tape_diagram": "icon", "picture_graph": "icon", "measurement_tool": "icon",
    "picture": "icon", "pattern_visual": "icon",
    "number_line": "index", "coordinate_plane": "index", "dot_plot": "index",
    "scatter_plot": "index", "line_graph": "index",
