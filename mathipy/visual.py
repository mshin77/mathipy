...
visual_model_definitions = {
    "number_line": "a line marked with ordered numeric values or tick marks (number line diagram); includes a double number line, which is two such lines in parallel",
    "coordinate_plane": "a grid with two perpendicular axes on which points, shapes, lines, or curves are plotted, including the graph of a function or equation; choose line_graph or scatter_plot when the plotted points are observed data values",
    ...
    "scatter_plot": "unconnected points plotted against labelled axes, whether or not the horizontal axis is numeric; choose line_graph when the points are joined by segments",
    ...
    "table": "rows and columns of values or labels without pictorial encoding; includes two-way tables and ratio tables",
    "polygon": "a closed plane figure bounded by straight sides, including a composite region made of joined straight-sided shapes; choose area_model when the rectangle is partitioned to show a product or parts of a whole, array when the figure is rows and columns of separate objects, coordinate_plane when the figure is drawn on axes, pattern_visual when it repeats or grows in a sequence, and solid_figure when it is drawn with depth",
    ...
    "circle_figure": "a circle or circular region studied as a shape, with radius, diameter, arc, or central angle marked; choose area_model when the circle is partitioned into parts of a whole, circle_graph when the sectors carry data values, and venn_diagram when circles overlap to show set membership",
    "angle_figure": "an angle, ray, line, or line segment construction where no closed region is the object of study; choose polygon when the lines close a region and measurement_tool when a protractor or ruler is shown against the figure",
