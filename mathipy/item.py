"""Unified feature extraction over any unit of text and its images."""
...
from mathipy.cohesion import cohesion_features, split_sentences
...
class MultimodalAnalyzer:
    """Run every analyzer over one unit and return a flat ``prefix_*`` dict.
    ...
    def extract(
        ...
        """Extract all features for one unit of text and its images.

        Args:
            text: The text to analyze.
            image_paths: Optional list of image file paths for visual features.
            units: Optional segments the text is composed of, such as turns.
                Cohesion is computed over these; sentences are used otherwise.
        ...
        segments = list(units) if units else split_sentences(text)
        row.update({f"cohesion_{k}": v for k, v in cohesion_features(segments).items()})
        row["n_units"] = len(segments)

        return row


ItemFeatureExtractor = MultimodalAnalyzer
