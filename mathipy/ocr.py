...
def _as_text(value: Any) -> str:
    """A multi-part item returns one string per sub-question rather than one string."""
...
def _parse_response(response_text: str) -> dict[str, Any]:
    ...
    if parsed is not None:
        for key in ("full_text", "image_description", "question_text"):
            parsed[key] = _as_text(parsed.get(key))
        if not parsed.get("content_type"):
            has_text = bool(parsed["full_text"])
            has_desc = bool(parsed["image_description"])
            parsed["content_type"] = (
...
class MultimodalOCR(VisionAPIClient):
    ...
    def extract(
        ...
        Returns:
            ...
            ``extraction_confidence``, and the provenance keys ``provider``,
            ``model``, and ``extracted_at``.
        """
    ...
    def _merge_image_results(
        ...
        base["parsed"] = base.get("parsed", True) and all(
            r.get("parsed", True) for r in image_results)
